...
tweets = []
for line in open('trainedData.txt').readlines():
...
tweets = []
for line in open('../Data/queryTweetsCleaned.txt').readlines():
    tweets.append(json.loads(line))
...
#positive tweets 
f = open('positiveQueryTweetsLR.txt', 'w')
for idx, [tweet_id, tweet_text] in enumerate(tweets):
    if y[idx] == 1:
        f.write(json.dumps(tweets[tweet_id - 1]) + '\r\n')        
f.close()

f = open('positiveQueryTweetsLRForWordCloud.txt', 'w')
for idx, [tweet_id, tweet_text] in enumerate(tweets):
    if y[idx] == 1:
        f.write(json.dumps(tweets[tweet_id - 1][1]))
f.close()
